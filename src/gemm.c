...
void gemm_ongpu_protected(int TA, int TB, int M, int N, int K, float ALPHA,
        float *A_gpu, int lda,
        float *B_gpu, int ldb,
        float BETA,
        float *C_gpu, int ldc,
        uint32_t *d_ES_a, uint32_t *d_ES_b, uint32_t *d_ES_c)
    {
        cudaError_t status = CutlassSgemmNN(N, M, K, ALPHA, B_gpu, ldb, A_gpu, lda, BETA, C_gpu, ldc, d_ES_a, d_ES_b, d_ES_c);
    }
