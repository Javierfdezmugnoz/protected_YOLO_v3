...
#include <../Aux_constructor/MMM_protected/MMM_protected.h>

// Included by JFdez 
#define MAX_LAYERS 13
#define MAX_TIMING_MEASUREMENTS 100
#define PERCENT_10 MAX_TIMING_MEASUREMENTS*1/10
#define PERCENT_20 MAX_TIMING_MEASUREMENTS*2/10
#define PERCENT_30 MAX_TIMING_MEASUREMENTS*3/10
#define PERCENT_40 MAX_TIMING_MEASUREMENTS*4/10
#define PERCENT_50 MAX_TIMING_MEASUREMENTS*5/10
#define PERCENT_60 MAX_TIMING_MEASUREMENTS*6/10
#define PERCENT_70 MAX_TIMING_MEASUREMENTS*7/10
#define PERCENT_80 MAX_TIMING_MEASUREMENTS*8/10
#define PERCENT_90 MAX_TIMING_MEASUREMENTS*9/10

// Generate an array and store in it the execution time of performing each MMM (each MMM corresponds a layer)
double execution_time[MAX_LAYERS][MAX_TIMING_MEASUREMENTS];
    ...
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    ...
    // ==============================================================================
    //  Memory allocation Execution Signature Array
    //  1) Define the pointers to h_ES_a,b,c
    // ==============================================================================
    ...
    // =============================================================
    //   2) Define the pointers to d_ES_a,b,c
    // =============================================================
    // Define pointers to d_ES_a,b,c and CRC lookup table in GPU (device)

    // Allocate d_ES_a,b,c in GPU
    cudaError_t result;
    result = cudaMalloc((uint32_t **) &net.d_ES_a, nBytes_ES);
    if (result != cudaSuccess) {
        printf("Failed to Allocate d_ES_a: %s",cudaGetErrorString(result));
        cudaFree(net.d_ES_a);
    }
    result = cudaMalloc((uint32_t **) &net.d_ES_b, nBytes_ES);
    if (result != cudaSuccess) {
        printf("Failed to Allocate d_ES_b: %s",cudaGetErrorString(result));
        cudaFree(net.d_ES_a);
        cudaFree(net.d_ES_b);
    }
    result = cudaMalloc((uint32_t **) &net.d_ES_c, nBytes_ES);
    if (result != cudaSuccess) {
        printf("Failed to Allocate d_ES_c: %s",cudaGetErrorString(result));
        cudaFree(net.d_ES_a);
        cudaFree(net.d_ES_b);
        cudaFree(net.d_ES_c);
    }
    ...
    // =============================================================
    //  3) Transfer h_ES_a... -> d_ES_a... (from host to device)
    // =============================================================
    result = cudaMemcpy(net.d_ES_a, h_ES_a, nBytes_ES, cudaMemcpyHostToDevice);
    if (result != cudaSuccess) {
        printf("Failed to copy h_ES_a matrix to d_ES_a: %s",cudaGetErrorString(result));
        cudaFree(net.d_ES_a);
        cudaFree(net.d_ES_b);
        cudaFree(net.d_ES_c);
    }
    ...
    result = cudaMemcpy(net.d_ES_b, h_ES_b, nBytes_ES, cudaMemcpyHostToDevice);
    if (result != cudaSuccess) 
    {
        printf("Failed to copy h_ES_a matrix to d_ES_b: %s",cudaGetErrorString(result));
        cudaFree(net.d_ES_a);
        cudaFree(net.d_ES_b);
        cudaFree(net.d_ES_c);
    }
    ...
    result = cudaMemcpy(net.d_ES_c, h_ES_c, nBytes_ES, cudaMemcpyHostToDevice);
    if (result != cudaSuccess) {
        printf("Failed to copy h_ES_a matrix to d_ES_c: %s",cudaGetErrorString(result));
        cudaFree(net.d_ES_a);
        cudaFree(net.d_ES_b);
        cudaFree(net.d_ES_c);
    }
    ...
    // ==============================================================================
        ...
        if (result != cudaSuccess) {
            printf("Failed to allocate Constant Memory: %s",cudaGetErrorString(result));
            cudaFree(net.d_ES_a);
            cudaFree(net.d_ES_b);
            cudaFree(net.d_ES_c);
    ...
    // Included by JFdez
    // ======================================================================
    //            TIME MEASUREMENTS: Definitions Chrono library
    // ======================================================================
    double start_time, end_time;
    static uint32_t current_measurement= 0u;


    while (1) {
        // Included by JFdez. Timing measurement experiment (full yolo-v3-tiny)
        ...
        if (save_labels)
        {
            printf("Entering in save_labels\n");
            char labelpath[4096];
        ...
        double end_time = get_time_point();
        // ==================================================================
        // Included by JFdez
        // ==================================================================
        if(current_measurement<MAX_TIMING_MEASUREMENTS)
        {
            execution_time[0u][current_measurement]= end_time - start_time;
        }

        /* ==============================================================================
            Brief: Store the timing measurements in a file (csv/xlsx)
        ==============================================================================*/
        if(MAX_TIMING_MEASUREMENTS==current_measurement)
        {
            FILE *p_file;
            char str_file_name[100u] = NAME"_timing.csv";
            printf("Transfering the info into %s.csv file\n",NAME);
            ...
            // Printf the values into the csv file:
            for (size_t idx_measurements = 10; idx_measurements < MAX_TIMING_MEASUREMENTS; idx_measurements++)
            {
                fprintf(p_file,"%10.0lf,",execution_time[0u][idx_measurements]);
            }
            fprintf(p_file,"\n");
            fclose(p_file);
        }
        ...
        case (MAX_TIMING_MEASUREMENTS-1):
            printf("[==============================] 100% \n");
            break;
