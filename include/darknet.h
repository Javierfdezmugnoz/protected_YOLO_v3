...
//#ifdef GPU
    ...
    uint32_t *d_ES_b;
    uint32_t *d_ES_c;
    // uint32_t EXTERNAL_ES;
    // uint32_t INTERMEDIATE_ES;
    // uint32_t INTERNAL_ES;
...
//#ifdef GPU
    ...
    // JFdez: I have the same definiion in layer struct. By the moment
    // I have decided that the ES is shared by all layers and, therefore,
    // to create in each layer a different pointer to the ES arrays has less
    // sense than create a single one in the network struct. Then: 
    uint32_t *d_ES_a;
    uint32_t *d_ES_b;
    uint32_t *d_ES_c;
